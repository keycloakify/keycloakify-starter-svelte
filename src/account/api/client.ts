...
export async function getUserProfile(): Promise<UserProfile> {
  ...
  const { kcHttpRelativePath, realm } = parseKeycloakIssuerUri(oidc.params.issuerUri)!;

  return fetch(`${kcHttpRelativePath ?? ''}/realms/${realm}/account/?userProfileMetadata=true`, {
    headers: {
...
export async function getI18nMessages(): Promise<I18nMessages> {
  ...
  const { kcHttpRelativePath, realm } = parseKeycloakIssuerUri((await getOidc()).params.issuerUri)!;
