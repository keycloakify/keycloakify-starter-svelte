...
import '../../../login-ui-svelte/tools/Array.prototype.every';
...
import { getPersistedPassword, persistPasswordOnFormSubmit } from './passwordRestoration';
...
function getInitialState(params: {
  ...
  const { passwordFieldName, passwordConfirmFieldName, makeConfirmationFieldHiddenAndAutoFilled, getErrors } = params;
...
function formStateSelector(params: { state: internal.State }): FormState {
  ...
  return {
    formFieldStates: state.formFieldStates.map(({ errors, hasLostFocusAtLeastOnce, attribute, value }) => ({
...
function createReducer(params: {
  ...
  const { passwordFieldName, passwordConfirmFieldName, getErrors, makeConfirmationFieldHiddenAndAutoFilled } = params;
  ...
  function getErrors(params: {
    ...
    check_password_policies: {
      ...
      check_password_policy_x: {
        ...
        errors.push({
          advancedMsgArgs: ['invalidPasswordMinLengthMessage' satisfies MessageKey_defaultSet, `${minLength}`] as const,
          source: {
            type: 'passwordPolicy',
            name: policyName,
          },
      ...
      check_password_policy_x: {
        ...
        errors.push({
          advancedMsgArgs: ['invalidPasswordMaxLengthMessage' satisfies MessageKey_defaultSet, `${maxLength}`] as const,
          source: {
            type: 'passwordPolicy',
            name: policyName,
          },
      ...
      check_password_policy_x: {
        ...
        errors.push({
          advancedMsgArgs: ['invalidPasswordNotUsernameMessage' satisfies MessageKey_defaultSet] as const,
          source: {
            type: 'passwordPolicy',
            name: policyName,
          },
      ...
      check_password_policy_x: {
        ...
        errors.push({
          advancedMsgArgs: ['invalidPasswordNotEmailMessage' satisfies MessageKey_defaultSet] as const,
          source: {
            type: 'passwordPolicy',
            name: policyName,
          },
    ...
    password_confirm_matches_password: {
      ...
      errors.push({
        advancedMsgArgs: ['invalidPasswordConfirmMessage' satisfies MessageKey_defaultSet] as const,
        source: {
...
const { createScopedDownUserProfileApi } = (() => {
  function readState(
    state_userProfile: ReturnType<NonNullable<ParamsOfGetNewPasswordApi['userProfileApi']>['getFormState']>,
    ...
    for (const name of ['username', 'email'] as const) {
      const formFieldState = state_userProfile.formFieldStates.find(({ attribute }) => attribute.name === name);
  ...
  function createScopedDownUserProfileApi(
    ...
    const state = readState(userProfileApi.getFormState());
