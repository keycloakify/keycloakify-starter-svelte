...
import { createObjectThatThrowsIfAccessed } from 'oidc-spa/tools/createObjectThatThrowsIfAccessed';
import { toFullyQualifiedUrl } from 'oidc-spa/tools/toFullyQualifiedUrl';
import { id } from 'oidc-spa/vendor/frontend/tsafe';
...
>(
  ...
  const loginOrGoToAuthServer = async (params: { redirectUrl: string | undefined }): Promise<never> => {
    ...
    const newUrl = new URL(
      (() => {
        ...
        return redirectUrl.startsWith('/') ? `${window.location.origin}${redirectUrl}` : redirectUrl;
      })(),
    );
