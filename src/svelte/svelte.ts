...
import { createOidc, OidcInitializationError, type Oidc, type ParamsOfCreateOidc } from 'oidc-spa';
import { Deferred } from 'oidc-spa/tools/Deferred';
...
export namespace OidcSvelte {
  ...
  export type LoggedIn<DecodedIdToken extends Record<string, unknown>> = Common & {
    ...
    subscribeToAutoLogoutCountdown: (tickCallback: (params: { secondsLeft: number | undefined }) => void) => {
...
): OidcSvelteApi<DecodedIdToken, ParamsOfCreateOidc extends { autoLogin?: true | undefined } ? true : false> {
