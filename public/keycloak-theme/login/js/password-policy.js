const policies = {
  ...
  upperCase: (policy, value) => {
    if (
      value.split('').filter((char) => char === char.toUpperCase() && char !== char.toLowerCase()).length < policy.value
    ) {
      return templateError(policy);
    }
  },
  lowerCase: (policy, value) => {
    if (
      value.split('').filter((char) => char === char.toLowerCase() && char !== char.toUpperCase()).length < policy.value
    ) {
      return templateError(policy);
    }
  },
