...
export function returnSuccess(result) {
  ...
  document.getElementById('signature').value = base64url.stringify(new Uint8Array(result.response.signature), {
    pad: false,
  });
  document.getElementById('credentialId').value = result.id;
  if (result.response.userHandle) {
    document.getElementById('userHandle').value = base64url.stringify(new Uint8Array(result.response.userHandle), {
      pad: false,
    });
  }
